...
def linear_shape(*input_shapes, kwargs):
    return (input_shapes[0][0], kwargs['out_features'])
...
def pool_shape(*input_shapes, kwargs):
    def _calc_output_dim(input_size, padding, dilation, kernel_size, stride) -> Expression:
        if padding == 'same':
            padding = kernel_size // 2
        ax = Floor(((input_size + padding * 2 - dilation * (kernel_size - 1) - 1) / stride) + 1)
    ...
    return (batch, input_shapes[0][1], output_height, output_width)
