...
class MarkdownBackend(DescriptionBackend):
    ...
    def generate(self, device) -> str:
        ...
        for mem in device.memories:
            memory_table += f"{mem.scope} | {mem.size} | {mem.latency} | {mem.management.value},{mem.coupling.value} | r: {mem.read_bw} w: {mem.write_bw} | r: {mem.read_energy} w: {mem.write_energy} i: {mem.idle_energy} | {mem.area} | r: {mem.read_port} w: {mem.write_port} rw: {mem.rw_port} | \n"
        text += memory_table + "\n"
