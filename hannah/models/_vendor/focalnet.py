...
def build_transforms(img_size, center_crop=False):
    ...
    Args:
      img_size:
      center_crop: (Default value = False)
    ...
    if center_crop:
        ...
        t.append(transforms.Resize(size, interpolation=str_to_interp_mode("bicubic")))
        t.append(transforms.CenterCrop(img_size))
    else:
        t.append(
            transforms.Resize(img_size, interpolation=str_to_interp_mode("bicubic"))
        )
...
def focalnet_tiny_srf(pretrained=False, **kwargs):
    ...
    Args:
      pretrained: (Default value = False)
      **kwargs:
...
if __name__ == "__main__":
    ...
    model = FocalNet(
        depths=[2, 2, 6, 2], embed_dim=96, focal_levels=[3, 3, 3, 3]
    ).cuda()
    print(model)
    model(x)
