...
def grouped_pointwise(input, out_channels):
    ...
    pw_l = grouped_conv2d(
        out, out_channels, kernel_size=1, stride=1, groups=pw_k.groups
    )
    out = add(pw_l, pw_k)
    return out
...
def spatial_correlation(input, out_channels, kernel_size, stride=1):
    return depthwise_conv2d(
        input, out_channels=out_channels, kernel_size=kernel_size, stride=stride
    )
...
def expand_reduce(input, out_channels, expand_ratio, kernel_size, stride):
    ...
    out = batch_norm(out)
    out = relu(out)
    out = spatial_correlation(
        out, kernel_size=kernel_size, stride=stride, out_channels=expanded_channels
    )
    out = batch_norm(out)
    out = relu(out)
...
def sandglass_block(input, out_channels, reduce_ratio, kernel_size, stride):
    in_channels = input.shape()[1]
    reduced_channels = Int(in_channels / reduce_ratio)
    out = depthwise_conv2d(
        input, out_channels=in_channels, kernel_size=kernel_size, stride=1
    )
    out = batch_norm(out)
    out = relu(out)
...
def pattern(input, stride, out_channels, kernel_size, expand_ratio, reduce_ratio):
    convolution = partial(
        conv_relu, stride=stride, kernel_size=kernel_size, out_channels=out_channels
    )
    red_exp = partial(
        reduce_expand,
...
def residual(input, main_branch_output_shape):
    ...
    out = conv2d(
        input, out_channels=out_channels, kernel_size=1, stride=stride, padding=0
    )
    out = batch_norm(out)
    out = relu(out)
    return out
...
def block(input, depth, stride, out_channels, kernel_size, expand_ratio, reduce_ratio):
    assert isinstance(
        depth, IntScalarParameter
    ), "block depth must be of type IntScalarParameter"
    out = input
    exits = []
