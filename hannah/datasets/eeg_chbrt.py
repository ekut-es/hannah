...
class EEGRTDataset(AbstractDataset):
    def __init__(self, config, file, split):
        super().__init__()
        self.config = config
    ...
    def splits(cls, config):
        ...
        val_set = EEGRTDataset(
            config,
            f"{config.data_folder}/{config.dataset}/preprocessed/{dataset_name}/retrain/{patient}.hdf5",
            "val",
        )
        test_set = EEGRTDataset(
            config,
            f"{config.data_folder}/{config.dataset}/preprocessed/{dataset_name}/test/{patient}.hdf5",
            "test",
        )
