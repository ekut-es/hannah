...
"""
    ...
    if features == "mel":
        ...
        data = librosa.core.stft(
            data, hop_length=hop_length, n_fft=n_fft, center=center
        )
        data = np.abs(data)
        data = data.astype(np.float32)
...
def main():
    ...
    for num, feature in enumerate(feature_set):
        ...
        if output_directory:
            with open(
                os.path.join(output_directory, feature + ".csv"), "w"
            ) as out_file:
        ...
        if feature == "raw":
            ...
            librosa.display.specshow(
                data, y_axis="log", x_axis="time", sr=sampling_rate
            )
