    ...
    def on_train_start(self):
        if self.hparams["fold_bn"] >= 0 and not self.bn_frozen:
            ...
            self.model = distiller.model_transforms.fold_batch_norms(
                self.model, dummy_input=self.example_input_array, inference=False
            )
        ...
        if self.hparams["compress"]:
            ...
            if len(self.trainer.optimizers) != 1:
                raise Exception(
                    "Compression is only available when using a single optimizer"
                )
            ...
            self.compression_scheduler = distiller.file_config(
                self.model, optimizer, self.hparams["compress"]
            )
