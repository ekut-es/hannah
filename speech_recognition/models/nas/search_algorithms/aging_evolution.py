...
class GPUTrainer:
    ...
    def evaluate(self, point):
        ...
        unstructured_sparsity = (
            self.trainer.config.pruning and not self.trainer.config.pruning.structured
        )
        resource_features = [
        ...
        log.info(
            f"Training complete: val_error={val_error:.4f}, test_error={test_error:.4f}, "
...
class AgingEvoSearch:
    ...
    def get_mo_fitness_fn(self):
        ...
        def fitness(i: EvaluatedPoint):
            ...
            normalised_features = [
                ...
                if c is not None
            ]  # bound = None means ignored objective
    ...
    def bounds_log(self, history_size=25):
        ...
        self.log.info(
            f"In bounds: {sum(within_bounds)} within "
    ...
    def evolve(self, point: ArchitecturePoint):
        ...
        if self.pruning:
            ...
            sparsity = np.clip(
                point.sparsity + incr,
                self.pruning.min_sparsity,
                self.pruning.max_sparsity,
            )
    ...
    def search(self, load_from: str = None, save_every: int = None):
        ...
        def should_submit_more(cap):
            return (
                len(self.history) + scheduler.pending_tasks() < cap
