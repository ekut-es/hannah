...
class CnnArchitecture(Architecture):
    ...
    * A resource graph model for computing resource usages using `.to_resource_graph()`.
    ...
    def _assemble_a_network(
        ...
        def tie_up_pending_outputs(outputs):
            ...
            for o in outputs:
                ...
                if downsampling_h > 1 or downsampling_w > 1:
                    o = pooling_layer(
                        o,
                        {"pool_size": (downsampling_h, downsampling_w), "type": "max"},
                    )
                xs.append(o)
            return add_layer(xs)
        ...
        for conv_block in self.architecture["conv_blocks"]:
            if conv_block["is_branch"]:
                ...
                for j, k in enumerate(conv_block["layers"]):
        ...
        for k in self.architecture["dense_blocks"]:
            x = dense_layer(x, k)

        final_dense = self.architecture.get("_final_dense", {})
        final_dense.update(
            {"units": 1 if num_classes == 2 else num_classes, "activation": None}
        )
    ...
    def to_keras_model(self, input_shape, num_classes, dropout=0.0, **kwargs):
        ...
        def conv_layer(x, k):
            if k["has_prepool"] and (x.shape[1] > 1 or x.shape[2] > 1):
            ...
            kernel_size = (
                1
                if k["type"] == "1x1Conv2D"
                else min(k["ker_size"], x.shape[1], x.shape[2])
            )
            stride = 1 if k["type"] == "1x1Conv2D" or not k["2x_stride"] else 2
            if k["type"] in ["Conv2D", "1x1Conv2D"]:
                conv = Conv2D(
                    filters=k["filters"],
                ...
                assert k["type"] == "DWConv2D"
                conv = DepthwiseConv2D(
                    kernel_size=kernel_size, strides=stride, padding="valid"
                )
                x = conv(x)
            if k["has_bn"]:
            ...
            if k["has_relu"]:
                x = ReLU()(x)
            k["_weights"] = [w.name for w in conv.trainable_weights]
            return x

        def pooling_layer(x, k):
            if isinstance(k["pool_size"], int):
                pool_h, pool_w = (k["pool_size"], k["pool_size"])
            else:
                pool_h, pool_w = k["pool_size"]
        ...
        def dense_layer(x, k):
            if k["activation"] is not None and dropout > 0.0:
                x = Dropout(dropout)(x)
            dense = Dense(units=k["units"], activation=k["activation"])
            x = dense(x)
            k["_weights"] = [w.name for w in dense.trainable_weights]
            return x
    ...
    def to_resource_graph(
        ...
        def process_pruned_weights(k):
            ...
            if "_weights" not in k:
                return None, None

            name = k["_weights"][0]  # Kernel matrix is the first entry
            if name not in pruned_weights:
                return None, None

            w = pruned_weights[name]
            channel_counts = tf.math.count_nonzero(
                tf.reshape(w, (-1, w.shape[-1])), axis=0
            )
        ...
        def conv_layer(x, k):
            ...
            stride = 1 if k["type"] == "1x1Conv2D" or not k["2x_stride"] else 2
            if k["type"] in ["Conv2D", "1x1Conv2D"]:
                filters, sparse_kernel_length = process_pruned_weights(k)
                x = Conv2D(
                    filters=filters or k["filters"],
                    kernel_size=kernel_size,
                    stride=stride,
                    padding="valid",
                    batch_norm=k["has_bn"],
                    activation="relu" if k["has_relu"] else None,
                    sparse_kernel_size=sparse_kernel_length,
                )(x)
            else:
                assert k["type"] == "DWConv2D"
                _, sparse_kernel_length = process_pruned_weights(k)
                x = DWConv2D(
                    kernel_size=kernel_size,
                    stride=stride,
                    padding="valid",
        ...
        def pooling_layer(x, k):
            if isinstance(k["pool_size"], int):
                ...
                pool_h, pool_w = k["pool_size"]
            pool_size = (min(pool_h, x.shape[1]), min(pool_w, x.shape[2]))
            return Pool(pool_size=pool_size, type=k["type"])(x)

        def dense_layer(x, k):
            units, sparse_kernel_size = process_pruned_weights(k)
            return Dense(
                units=units or k["units"],
                preflatten_input=True,
                activation=k["activation"],
                sparse_kernel_size=sparse_kernel_size,
            )(x)
