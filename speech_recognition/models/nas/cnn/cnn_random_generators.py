...
def random_conv_layer_type(
    block_idx, layer_idx, layer_type, relu_prob=0.9, pre_pool_prob=0.25
):
...
def random_conv_block(block_idx, num_layers=None):
    ...
    block = {
        "is_branch": False
        if block_idx == 0
        else schema[f"conv{i}-is-branch"].uniform_random_value(),
        "layers": [],
    }
...
def random_arch(pooling_prob=0.9):
    ...
    """
    schema = get_schema()
    arch = {"conv_blocks": [], "pooling": None, "dense_blocks": []}
