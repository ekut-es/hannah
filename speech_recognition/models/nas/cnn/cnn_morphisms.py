...
from .cnn_random_generators import (
    random_conv_block,
    random_conv_layer,
    random_pooling,
    random_dense_block,
    random_conv_layer_type,
)


def remove_random_conv_block(arch):
    return remove_conv_block(arch, idx=np.random.randint(len(arch["conv_blocks"])))
...
def remove_conv_block(arch, idx):
    ...
    block_after_is_branch = (idx < num_blocks - 1) and arch["conv_blocks"][idx + 1][
        "is_branch"
    ]
...
def change_conv_block_type(arch, block_idx):
    ...
    arch = deepcopy(arch)
    arch["conv_blocks"][block_idx]["is_branch"] = not arch["conv_blocks"][block_idx][
        "is_branch"
    ]
    return [arch]
    ...
    for i, conv_block in enumerate(parent["conv_blocks"]):
        ...
        for j, conv_layer in enumerate(conv_block["layers"]):
            ...
            if conv_layer["type"] in ["Conv2D", "DWConv2D"]:
                ...
                morphs.append(
                    change_conv_layer_boolean_property(parent, i, j, "2x_stride")
                )
