...
class OperatorDesc:
    ...
    def _add_weight(self, shape, suffix="weight", sparse_size=None):
        t = self.graph.add_tensor(
            f"{self.name}_{suffix}", shape, is_constant=True, sparse_size=sparse_size
        )
...
class Graph:
    ...
    def add_tensor(
        self, proposed_name, shape, is_constant=False, producer=None, sparse_size=None
    ):
        name = _disambiguate_name(proposed_name, self.tensors)
        t = TensorDesc(
            name, shape, self.element_size, is_constant, producer, sparse_size
        )
...
def _get_current_graph() -> Graph:
    ...
    if _current_graph is None:
        raise ValueError(
            "Wrap your model assembling code in a `with g.as_current()` block"
        )
