...
class Conv2D(OperatorDesc):
    ...
    def __init__(
        ...
        padding: str = "valid",
        sparse_kernel_size: Optional[int] = None,
        name: str = None,
    ):
    ...
    def __call__(self, x: TensorDesc):
        ...
        self._add_weight(
            shape=(self.kernel_size, self.kernel_size, in_channels, self.num_filters),
            suffix="weight",
...
class DWConv2D(OperatorDesc):
    ...
    def __call__(self, x: TensorDesc):
        ...
        batch_size, h, w, in_channels = x.shape
        self._add_weight(
            shape=(self.kernel_size, self.kernel_size, in_channels, 1),
            suffix="weight",
            sparse_size=self.sparse_kernel_size,
        )
        if self.use_bias:
...
class Pool(OperatorDesc):
    def __init__(
        self, pool_size: Union[int, Tuple[int, int]], type: str, name: str = None
    ):
    ...
    def __call__(self, x: TensorDesc):
        ...
        assert (
            pool_h <= h and pool_w <= w
        ), f"Can't apply {self.pool_size} pooling to {x.shape}"
