...
def peak_memory_usage(g: Graph, exclude_weights=True, exclude_inputs=True):
    ...
    def mem(tensors):
        ...
        if constants:
            upstream_mem_use, op_order = mem(
                frozenset(t for t in tensors if t.producer is not None)
            )
        ...
        for t in tensors:
            ...
            if isinstance(t.producer, Add) and any(
                i.shape == t.shape and last_use_point(i) for i in inputs
            ):
    ...
    if len(g.outputs) == 0:
        raise ValueError(
            "Provided graph has no outputs. Did you call `g.add_output(...)`?."
        )
...
def model_size(g: Graph, sparse=False):
    return sum(
        x.size if not sparse else (x.sparse_size or x.size)
        for x in g.tensors.values()
        if x.is_constant
    )
...
def inference_latency(
    g: Union[Graph, OperatorDesc, List[OperatorDesc]],
    mem_access_weight=0,
    compute_weight=1,
):
