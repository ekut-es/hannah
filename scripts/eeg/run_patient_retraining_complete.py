...
try:
    ...
    DATASET_NAME = sys.argv[2]      # Ex. samp256, samp256halfsec !! dataset used for training the main model stored in `trained_model/base_model/{MODEL_NAME}/`
    DATA_FOLDER = sys.argv[3]
    CKPT=sys.argv[4]                # Checkpoint file of pretrained model
except:
    raise EnvironmentError("Model name or dataset name missing")

try: EXPT_NAME = sys.argv[3]        # custom experiment name as an identifier. Used while storing results generated in this file
except: EXPT_NAME = f"{datetime.now():%d%m}" 
    

patient_list = os.listdir(f"{DATA_FOLDER}/chbmit/preprocessed/{DATASET_NAME}/retrain")
try: patient_list.remove(".ipynb_checkpoints")
...
plt.savefig(f"results/combined_confusion_matrix_{MODEL_NAME}_{DATASET_NAME}_{EXPT_NAME}.pdf")